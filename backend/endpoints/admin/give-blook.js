export default {
    ...
    handler: async (req, res) => {
        try {
            ...
            const { username, blookName } = req.body;
            if (!username || !blookName) return res.status(400).json({ error: 'Missing fields' });
            ...
            await database.query("INSERT INTO blooks (user_id, blook_name, rarity) VALUES (?, ?, 'Common')", { replacements: [target.id, blookName] });
